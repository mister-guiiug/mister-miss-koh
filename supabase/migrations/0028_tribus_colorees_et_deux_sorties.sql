-- ╔══════════════════════════════════════════════════════════════════════════╗
-- ║ mister-miss-koh — Les tribus ont une couleur, et l'on peut sortir deux   ║
-- ║ fois                                                                     ║
-- ║                                                                          ║
-- ║ Trois manques, tous révélés par la page All Stars du 23/09/2026, et tous ║
-- ║ vrais bien au-delà d'elle — relevé sur les dix-huit pages le même jour.  ║
-- ╚══════════════════════════════════════════════════════════════════════════╝
--
-- 1. LA COULEUR DES TRIBUS N'ARRIVAIT PAS. `teams.colour` existe depuis 0001,
--    et rien ne l'écrivait : la source dit pourtant, sous son tableau, que
--    « Taboga » est la tribu jaune et « Sebako » la rouge, et chaque ligne de
--    la colonne « Tribu » porte sa pastille. L'extraction 12 la lit et la
--    propose une fois par tribu (entité `team`) ; cette fonction la publie.
--
-- 2. LE JOUR DU CONSEIL N'ARRIVAIT PAS NON PLUS. Les appartenances sont
--    datées en JOURS — « Taboga (jour 9 – ) » —, l'anti-spoiler raisonne en
--    ÉPISODES, et la correspondance manquait. Elle est pourtant dans la
--    source : la colonne « Départ » du déroulement date le conseil de chaque
--    soirée (« Jour 11 »), et l'extraction la lit depuis sa version 2
--    (`departureDay`). Elle va désormais dans `episodes.day_end`. Rien n'est
--    converti ici : un jour reste un jour, et c'est l'application qui en
--    déduit, prudemment, à partir de quel épisode un séjour peut se montrer.
--
-- 3. UN CANDIDAT NE POUVAIT SORTIR QU'UNE FOIS — `unique (season_contestant_id)`
--    sur `departures`. Or on revient : Maxime, éliminé au premier conseil,
--    rentre dans Sebako au jour 9 ; Charlotte, sortie avec son binôme à
--    l'épisode 3, revient dans Taboga puis est éliminée à l'épisode 5. Relevé
--    du 23/09/2026 : une même personne éliminée deux fois dans la matrice des
--    votes sur quatorze pages sur seize. Avec une seule ligne, la seconde sortie
--    ÉCRASAIT la première : l'épisode 3 perdait sa sortie, et l'anti-spoiler
--    montrait Charlotte « en jeu » à qui en était là. Une sortie se rattache
--    désormais à sa SOIRÉE : une ligne par (candidat, épisode).
--
-- ET UN MOT FAUX, CORRIGÉ AU PASSAGE : une colonne à zéro voix qu'aucun vote
-- de la soirée ne cause n'est pas un départ de binôme. C'est un abandon, une
-- évacuation, une élimination à l'arène — la matrice ne dit pas lequel. Ces
-- colonnes existent dans quinze saisons sur seize ; elles étaient toutes
-- publiées en `linked_pair`, et l'application écrivait « part avec son
-- binôme » dans des saisons sans binômes. Elles deviennent `other`.

-- ── Une sortie par soirée, et plus une par candidat ─────────────────────────
alter table departures drop constraint departures_season_contestant_id_key;

create unique index departures_une_par_soiree
  on departures (season_contestant_id, episode_id);

comment on index departures_une_par_soiree is
  'Une sortie par candidat ET par épisode : on revient de l''exil, et l''on peut ressortir (0028). Le retour se lit dans les séjours en tribu, datés en jours.';

comment on column episodes.day_end is
  'Jour du conseil qui clôt l''épisode, tel que la colonne « Départ » du déroulement l''écrit (« Jour 11 »). `day_start` reste vide : la source ne le donne pas.';

-- ── La couleur ne peut être qu'une couleur ──────────────────────────────────
--
-- Elle finit dans un attribut `style` de l'application. L'extraction ne laisse
-- déjà passer qu'un `#rrggbb` ; la base le refuse aussi, pour qu'aucun autre
-- chemin d'écriture ne puisse y glisser `url(…)`. `not valid` : les lignes
-- existantes n'ont jamais eu de couleur, et la contrainte n'a pas à les relire.
alter table teams
  add constraint teams_colour_hexadecimale
  check (colour is null or colour ~ '^#[0-9a-f]{6}$') not valid;
        ...
        if found then
          ...
          if found then
            ...
            insert into departures (season_contestant_id, episode_id, council_id,
                                    round_id, kind,
                                    source_document_id, validation_status, published_at)
            values (v_contestant, v_episode, v_council, v_round, v_kind,
                    v_run.source_document_id, 'published', now())
            returning id into v_id;
          end if;
