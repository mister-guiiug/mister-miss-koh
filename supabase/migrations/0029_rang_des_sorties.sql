-- ╔══════════════════════════════════════════════════════════════════════════╗
-- ║ mister-miss-koh — Une sortie sans scrutin garde sa place dans la soirée  ║
-- ╚══════════════════════════════════════════════════════════════════════════╝
--
-- CE QUI SE VOYAIT. Épisode 4 d'All Stars, publié le 23/09/2026 : les quatre
-- sorties de l'arène (Joana, Moussa, Naoil, Jacques) s'affichaient APRÈS
-- l'élimination de Lola. La source les place avant : ce sont les colonnes 1 à
-- 4 de la soirée, et le conseil de Lola la cinquième.
--
-- LA CAUSE. L'extraction numérote chaque colonne du tableau des votes dans sa
-- soirée (`roundNumber`), sorties sans scrutin comprises. Un tour publié garde
-- ce numéro (`council_rounds.round_number`). Une sortie sans scrutin n'a pas
-- de tour — 0009 : ce serait un scrutin fantôme —, et son numéro se PERDAIT à
-- la publication. L'application la rangeait donc après le dernier conseil :
-- juste pour un départ de binôme, qui suit l'élimination qui le cause, faux
-- pour tout le reste.
--
-- L'AMPLEUR. Relevé du 24/09/2026 sur les pages du catalogue : 102 colonnes
-- sans scrutin, dans quinze saisons, et 65 PRÉCÈDENT un tour de leur soirée —
-- abandons, évacuations, l'arène, les épreuves de la finale. Plusieurs se
-- suivent souvent (quatre à l'arène, trois à l'épisode 5 de « La Tribu
-- maudite ») : leur ordre entre elles ne se déduit d'aucun tour publié. Il
-- faut donc le GARDER, pas le reconstituer.
--
-- LE CORRECTIF. `departures.round_number` garde la colonne de la source ;
-- `publish_run` l'écrit pour toute sortie, avec ou sans scrutin ; et
-- `recaler_rangs_des_sorties` le rend aux sorties déjà publiées.

-- ── La colonne ──────────────────────────────────────────────────────────────
alter table departures
  add column if not exists round_number integer
    constraint departures_rang_positif check (round_number > 0);

comment on column departures.round_number is
  'Rang de la sortie dans sa soirée : la colonne qu''elle occupe dans le tableau des votes de la source (0029). Une sortie sans scrutin n''a pas de tour en base, et c''est ce rang qui la place avant ou après le conseil. Vide : rang inconnu, la sortie se range après les autres.';

-- ── La publication écrit le rang ────────────────────────────────────────────
--
-- Celle de 0028, à un seul changement près : la branche des sorties écrit
-- `round_number` (« LE RANG DANS LA SOIRÉE », plus bas).
...
declare
  ...
  v_holder        jsonb;
  v_rang          integer;
  v_season_shot   boolean := false;
  v_kind          departure_kind;
begin
  ...
  -- la première clé étrangère manquante. La tribu passe AVANT les candidats :
  -- elle naît avec sa couleur, et les séjours la retrouvent par son nom.
  for v_diff in
    ...
    order by case entity
      when 'season' then 0
      when 'team' then 1
      when 'season_contestant' then 2
      when 'episode' then 3
      when 'council_round' then 4
      when 'council_vote' then 5
      when 'advantage' then 6
      else 9 end,
      natural_key
  loop
    v_payload := coalesce(v_diff.after_value, '{}'::jsonb);

    -- ── La saison : son lieu de tournage ─────────────────────────────────
    --
    -- Une seule ligne, quatre colonnes, et la même règle que tout le reste :
    -- l'extraction propose, la relecture valide, la publication écrit. La
    -- photo de la saison n'est prise qu'une fois par publication — deux
    -- photos de la même ligne, reposées dans l'ordre inverse, laisseraient
    -- la saison « publiée » après un retour arrière qui devait la cacher.
    if v_diff.entity = 'season' then
      if not v_season_shot then
        v_snapshot := v_snapshot || jsonb_build_array(jsonb_build_object(
          't', 'seasons', 'id', v_season.id, 'before', row_snapshot('seasons', v_season.id)));
        v_season_shot := true;
      end if;
      ...
      update seasons
         set location_name = nullif(v_payload ->> 'locationName', ''),
             location_page_title = nullif(v_payload ->> 'locationPageTitle', ''),
             location_lat = nullif(v_payload ->> 'locationLat', 'null')::double precision,
             location_lon = nullif(v_payload ->> 'locationLon', 'null')::double precision,
             source_document_id = v_run.source_document_id,
             updated_at = now()
       where id = v_season.id;

    -- ── Tribu : son nom et sa couleur ────────────────────────────────────
    --
    -- UNE DIFFÉRENCE PAR TRIBU, pas une par candidat : la couleur est un fait
    -- de la tribu. Une tribu déjà créée par un séjour garde son identifiant —
    -- les appartenances et les résultats d'épreuve pointent dessus — et
    -- reçoit sa couleur ; photographiée avant, pour que le retour arrière la
    -- lui retire.
    elsif v_diff.entity = 'team' then
      v_name := v_payload ->> 'name';
      if v_name is null or v_name = '' then
        raise exception 'différence % sans nom de tribu', v_diff.natural_key;
      end if;
      -- Ce que la contrainte refuserait, on ne l'écrit pas : une couleur
      -- illisible vaut « pas de couleur », pas un lot entier en échec.
      v_colour := v_payload ->> 'colour';
      if v_colour is not null and v_colour !~ '^#[0-9a-f]{6}$' then
        v_colour := null;
      end if;

      select id into v_team from teams
       where season_id = v_season.id and name = v_name;
      if found then
        v_before := row_snapshot('teams', v_team);
        update teams
           set colour = v_colour,
               source_document_id = v_run.source_document_id,
        ...
        v_before := null;
        insert into teams (season_id, name, kind, colour, source_document_id,
                           validation_status, published_at)
        values (v_season.id, v_name, team_kind_of(v_name), v_colour,
                v_run.source_document_id, 'published', now())
        returning id into v_team;
      end if;

      v_snapshot := v_snapshot || jsonb_build_array(jsonb_build_object(
        't', 'teams', 'id', v_team, 'before', v_before));

    -- ── Candidat ─────────────────────────────────────────────────────────
    elsif v_diff.entity = 'season_contestant' then
      ...
      -- c'est ce que la source dit — « Ikalu (jour 2 – 5) ». Elles ne sont
      -- pas converties en épisodes : c'est l'application qui, avec le jour
      -- de chaque conseil (`episodes.day_end`), décide à partir de quand un
      -- séjour peut se montrer sans rien divulguer.
      --
    ...
    -- ── Épisode ──────────────────────────────────────────────────────────
      ...
      -- `day_end` : le jour du conseil de la soirée (« Départ : Jour 11 »).
      -- C'est lui qui permet de dire à partir de quel épisode « Taboga (jour
      -- 9 – ) » peut se montrer.
      if found then
        update episodes
           set air_date = nullif(v_payload ->> 'airDate', 'null')::date,
               day_end = nullif(v_payload ->> 'departureDay', 'null')::integer,
               source_document_id = v_run.source_document_id,
               validation_status = 'published',
               published_at = now()
         where id = v_id;
      else
        insert into episodes (season_id, number, air_date, day_end, source_document_id,
                              validation_status, published_at)
        values (v_season.id, (v_payload ->> 'number')::integer,
                nullif(v_payload ->> 'airDate', 'null')::date,
                nullif(v_payload ->> 'departureDay', 'null')::integer,
                v_run.source_document_id, 'published', now())
        returning id into v_id;
      end if;
    ...
    -- ── Tour de conseil ──────────────────────────────────────────────────
      ...
      -- UNE SORTIE SANS SCRUTIN N'EST PAS UN SCRUTIN.
      --
      -- L'extraction produit un tour `linked` parce que la SOURCE lui donne une
      -- colonne — le tableau des votes en réserve une par sortie, et le départ
      ...
      -- ── La sortie qui découle du tour ─────────────────────────────────
      --
      -- « Éliminé », « part avec son binôme », « quitte le jeu sans vote » :
      -- trois sorties distinctes, et les deux dernières se lisent ZÉRO VOTE.
      -- Ce qui les sépare est la CAUSE que l'extraction a lue — une sortie
      -- liée suit l'élimination de sa propre soirée. Sans elle, c'est un
      -- abandon, une évacuation, une élimination à l'arène : la matrice ne
      -- dit pas lequel, donc `other`, et non un binôme qui n'existe pas.
      v_name := v_payload ->> 'eliminated';
      if v_name is not null and v_name <> '' and (v_payload ->> 'kind') <> 'annulled' then
        v_kind := case
          when (v_payload ->> 'kind') = 'linked' and (v_payload ->> 'causedBy') is not null
            then 'linked_pair'
          when (v_payload ->> 'kind') = 'linked' then 'other'
          else 'vote'
        end::departure_kind;
        ...
        if found then
          -- LA SORTIE DE CETTE SOIRÉE, pas « la » sortie du candidat : celle
          -- d'un épisode antérieur reste ce qu'elle était.
          select id into v_id from departures
           where season_contestant_id = v_contestant and episode_id = v_episode;
          v_before := case when found then row_snapshot('departures', v_id) end;

          -- LE RANG DANS LA SOIRÉE (0029) : la colonne de la source, écrite
          -- pour toute sortie. Celle qui n'a pas de tour n'a que lui pour dire
          -- si elle vient avant ou après le conseil.
          if found then
            update departures
               set council_id = v_council, round_id = v_round, kind = v_kind,
                   round_number = (v_payload ->> 'roundNumber')::integer,
                   -- Une cause ne survit pas à un changement de nature.
                   caused_by_departure_id = case when v_kind = 'linked_pair'
                                                 then caused_by_departure_id end,
                   source_document_id = v_run.source_document_id,
                   validation_status = 'published', published_at = now()
             where id = v_id;
          else
            ...
            insert into departures (season_contestant_id, episode_id, council_id,
                                    round_id, round_number, kind,
                                    source_document_id, validation_status, published_at)
            values (v_contestant, v_episode, v_council, v_round,
                    (v_payload ->> 'roundNumber')::integer, v_kind,
                    v_run.source_document_id, 'published', now())
            returning id into v_id;
          end if;
          ...
          if v_kind = 'linked_pair' then
            -- L'EXTRACTION nomme la cause ; ici on ne fait que la retrouver,
            -- dans la MÊME soirée : le partenaire a pu sortir ailleurs aussi.
            -- Un nom qui ne désigne personne, ou qui désigne la personne
            -- elle-même, n'écrit rien : mieux vaut un duo absent qu'un faux.
            v_partner := null;
            v_cause := null;
            select sc.id into v_partner from season_contestants sc
             where sc.season_id = v_season.id
               and sc.display_name = (v_payload ->> 'causedBy');
            if v_partner is not null then
              select d.id into v_cause from departures d
               where d.season_contestant_id = v_partner and d.episode_id = v_episode;
            end if;
  ...
  -- LE DOCUMENT SOURCE DIT QUELLE RÉVISION EST EN LIGNE. `last_seen_revision`
  -- et `last_seen_at` restaient vides : l'application, qui les lit pour
  -- afficher « révision N, lue le … », n'avait rien à montrer. La révision
  -- publiée est celle de l'exécution ; la date, celle où la page a été lue.
  update source_documents
     set last_seen_revision = coalesce(v_run.source_revision, last_seen_revision),
         last_seen_at = coalesce(v_run.started_at, now())
...
-- ── Les sorties déjà publiées ───────────────────────────────────────────────
--
-- Une fonction plutôt qu'une instruction : c'est ce qui permet de l'éprouver.
-- Jouée ici sur une base neuve, une instruction ne toucherait aucune ligne, et
-- rien ne dirait si elle range juste. `p_season_id` borne le recalage à une
-- saison ; vide, il les parcourt toutes. Elle ne remplit que des rangs VIDES
-- et peut être rejouée sans rien déplacer.
create or replace function public.recaler_rangs_des_sorties(p_season_id uuid default null)
returns integer
...
declare
  ...
  v_sans    integer;
begin
  -- Une sortie par scrutin : son tour porte déjà le rang.
  update departures d
     set round_number = cr.round_number
    from council_rounds cr, season_contestants sc
   where cr.id = d.round_id
     and sc.id = d.season_contestant_id
     and (p_season_id is null or sc.season_id = p_season_id)
     and d.round_number is null;
  get diagnostics v_scrutin = row_count;

  -- Une sortie sans scrutin : la colonne `linked` qui l'a publiée, relue dans
  -- la dernière exécution PUBLIÉE de sa page — celle que l'import tient pour
  -- l'état publié (`loadPublished`). Rapprochée comme `publish_run` la
  -- rapproche : même saison, même épisode, même prénom. Deux colonnes pour la
  -- même personne dans la même soirée ne disent pas laquelle a été publiée :
  -- le rang reste vide plutôt que faux.
  update departures d
     set round_number = x.round_number
    from (
      select sc.id as season_contestant_id, e.id as episode_id,
             min((ir.payload ->> 'roundNumber')::integer) as round_number
        from seasons s
        cross join lateral (
        ...
        join import_records ir
         ...
         and ir.payload ->> 'kind' = 'linked'
        join episodes e
          on e.season_id = s.id
         and e.number = (ir.payload ->> 'episodeNumber')::integer
        join season_contestants sc
          on sc.season_id = s.id
         and sc.display_name = ir.payload ->> 'eliminated'
       where p_season_id is null or s.id = p_season_id
       group by sc.id, e.id
      having count(*) = 1
    ) x
   where d.season_contestant_id = x.season_contestant_id
     and d.episode_id = x.episode_id
     and d.round_id is null
     and d.round_number is null;
  get diagnostics v_sans = row_count;

  return v_scrutin + v_sans;
end $$;

revoke all on function public.recaler_rangs_des_sorties(uuid) from public, anon, authenticated;

comment on function public.recaler_rangs_des_sorties(uuid) is
  'Rend leur rang dans la soirée aux sorties publiées avant 0029 : celui de leur tour, ou celui de leur colonne dans la dernière exécution publiée. Ne remplit que des rangs vides ; rend le nombre de sorties recalées.';

select public.recaler_rangs_des_sorties();
