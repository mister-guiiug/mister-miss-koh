-- ╔══════════════════════════════════════════════════════════════════════════╗
-- ║ mister-miss-koh — Les duos, autant que la source en dit                   ║
-- ║                                                                          ║
-- ║ LA SOURCE NE LISTE PAS LES DUOS. Vérifié le 05/09/2026 : la section       ║
-- ║ « Nouveautés » énonce la RÈGLE en prose — « si l'un des deux est éliminé  ║
-- ║ lors d'un conseil, son partenaire quitte lui aussi immédiatement » — et   ║
-- ║ ne nomme aucun couple. Le tableau des candidats n'a pas de colonne de     ║
-- ║ duo. Les neuf duos de la saison ne sont donc écrits nulle part.           ║
-- ║                                                                          ║
-- ║ CE QUE LA STRUCTURE DIT, ELLE, SANS AMBIGUÏTÉ. Le tableau des votes donne ║
-- ║ une colonne par scrutin : à l'épisode 1, un tour de vote qui élimine      ║
-- ║ Maxime, puis une colonne « départ lié » où part Joana. Cette seconde      ║
-- ║ colonne n'existe QUE parce que la première a eu lieu. La sortie de Joana  ║
-- ║ est donc causée par celle de Maxime — c'est le sens de la colonne, pas    ║
-- ║ une déduction — et cela nomme leur duo.                                   ║
-- ║                                                                          ║
-- ║ D'OÙ LA LIMITE, QUI EST ÉCRITE ET ASSUMÉE : un duo n'est connu que        ║
-- ║ lorsqu'un départ le révèle. Deux duos sur neuf à ce jour. Les sept autres ║
-- ║ ne sont pas « manquants » : ils ne sont écrits nulle part, et les         ║
-- ║ deviner à partir des vainqueurs d'épreuve serait une invention.           ║
-- ║                                                                          ║
-- ║ `formed_episode_number` reste NULL. La prose dit « dès le début du jeu » ;║
-- ║ le pipeline n'importe pas de prose, et un duo révélé à l'épisode 2 n'a    ║
-- ║ pas été formé à l'épisode 2.                                             ║
-- ╚══════════════════════════════════════════════════════════════════════════╝

-- Les binômes sont publiables comme le reste : la photo doit pouvoir les
-- reposer.
...
language sql immutable as $$
  select p_table in (
    ...
    'advantages', 'advantage_holders', 'pairs'
  )
$$;
