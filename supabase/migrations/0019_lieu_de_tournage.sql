-- ╔══════════════════════════════════════════════════════════════════════════╗
-- ║ mister-miss-koh — Le lieu de tournage                                     ║
-- ║                                                                          ║
-- ║ L'infobox de la page source dit où la saison se tourne — « Archipel des  ║
-- ║ Perles (Panama) » — et la page du lieu porte des coordonnées. Ni l'un ni  ║
-- ║ l'autre n'entraient dans le référentiel : l'application ne pouvait ni    ║
-- ║ nommer le lieu ni le montrer sur une carte.                              ║
-- ║                                                                          ║
-- ║ Quatre colonnes sur `seasons`, et une entité de plus dans le pipeline :   ║
-- ║ `season`, une différence par saison, relue et publiée comme les autres  ║
-- ║ (version 5 du modèle intermédiaire). Le nom est le texte de la source ;  ║
-- ║ le titre de page est celui qu'on a géolocalisé — l'archipel, pas le      ║
-- ║ pays — pour que la carte pointe l'endroit le plus précis que la source   ║
-- ║ cite. Les coordonnées viennent de l'API (`prop=coordinates`), jamais    ║
-- ║ d'une supposition.                                                       ║
-- ╚══════════════════════════════════════════════════════════════════════════╝

alter table seasons
  add column location_name text,
  add column location_page_title text,
  ...
  -- Un point se donne entier ou pas du tout : une latitude sans longitude
  -- ne se place nulle part.
  add constraint seasons_location_point check (
    (location_lat is null) = (location_lon is null)
  );

comment on column seasons.location_name is
  'Lieu de tournage tel que la source l''écrit (« Archipel des Perles (Panama) »).';
comment on column seasons.location_page_title is
  'Titre de la page de lieu géolocalisée — la plus précise que la source cite.';
comment on column seasons.location_lat is
  'Latitude de la page de lieu (API MediaWiki, prop=coordinates), degrés décimaux.';
comment on column seasons.location_lon is
  'Longitude de la page de lieu (API MediaWiki, prop=coordinates), degrés décimaux.';
    ...
    -- ── Tour de conseil ──────────────────────────────────────────────────
      ...
      if v_name is not null and v_name <> '' and (v_payload ->> 'kind') <> 'annulled' then
        ...
        if found then
          ...
          if (v_payload ->> 'kind') = 'linked'
             and (v_payload ->> 'causedBy') is not null then
            ...
            if v_partner is not null then
              select d.id into v_cause from departures d
               where d.season_contestant_id = v_partner;
            end if;
