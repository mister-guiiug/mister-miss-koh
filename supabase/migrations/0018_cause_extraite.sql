-- ╔══════════════════════════════════════════════════════════════════════════╗
-- ║ mister-miss-koh — La cause d'un départ lié vient de l'EXTRACTION           ║
-- ║                                                                          ║
-- ║ 0015 cherchait, pour un départ lié, l'élimination du même épisode — dans  ║
-- ║ une requête SQL. Ça marchait, et c'était au mauvais endroit : personne ne ║
-- ║ relit une jointure enfouie dans une fonction, alors que tout le pipeline  ║
-- ║ est bâti pour qu'un relecteur VOIE ce qui va être écrit.                  ║
-- ║                                                                          ║
-- ║ L'extraction nomme donc la cause (`causedBy`, version 4 du modèle         ║
-- ║ intermédiaire). Elle apparaît dans la différence proposée, elle passe par ║
-- ║ la relecture, et la publication ne fait plus que traduire un nom en       ║
-- ║ identifiant. La règle « le binôme suit l'éliminé » n'est ni supposée ni   ║
-- ║ codée : elle est LUE dans la structure du tableau des votes.              ║
-- ╚══════════════════════════════════════════════════════════════════════════╝
